...
<mark>So agent $$a$$ makes offers in odd timestamps, while $$b$$ makes offers in even timestamps</mark>. This is the core of "Alternating" Offer protocol.
...
Rubinstein shows that it is possible to compute a <mark>subgame perfect equilibrium</mark>:
...
Property: <mark>the final agreement found via this algorithm is a pareto optimal agreement</mark> which means that there is no another agreement that is not worse for one agent and strictly better for the other one.

<mark>it finds an agreements which is a Nash BARGAINING solution</mark>. It is an agreement that maximizes the product of the two utilities.